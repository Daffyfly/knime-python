...
class CommandMessage(object):
    ...
    # Constructor.
    # @param header a header containing annotations in the from @<key>=<value>
    #               @id and @command are required.
    # @param payload the message payload that should be encoded using a PayloadHandler.
    ...
    # Checks the options for forbidden signs. Those are @ and = used in the 
    # annotation syntax.
...
# Message indicating that execution of a source code snippet is done.
# Provides stdout and stderr output as payload-strings.
...
# Generic message transporting a single integer value in the payload.
...
# Generic message transporting a single string value in the payload.
...
# Generic message transporting a single bytes value in the payload.
